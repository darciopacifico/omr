...
public class UploadArquivosAction extends BaseAction {
		...
		for (TpValidVO tpValidVO : tiposValidacaoPossiveis) {
			for (String selecionado : strTiposSelecionados) {
				...
				if(COMPLETA.equals(selecionado)){
					return carregaTodosOsTipoValidacaoValidos(COMPLETA);
				} 
	 ...
	 * @return
	 */
	private List<TpValidVO> carregaTodosOsTipoValidacaoValidos(String COMPLETA) {
		
		BigDecimal tarifaCompleta = new BigDecimal(0);
		List<TpValidVO> tiposSelecionados;
		
		
		List<TpValidVO> validacoes = pedidoValidacaoServer.getListaValidacoesTodas();
		
		
		tiposSelecionados = new ArrayList<TpValidVO>();
		for (TpValidVO tpValidacao : validacoes) {
			if(!COMPLETA.equals(tpValidacao.getId().toString())) {
				tpValidacao.setTarifaFk(new TarifaVO());
				// Zera o valor das tarifas para não ser contabilizar;
				tiposSelecionados.add(tpValidacao);
			} else {
				// Guarda o valor da Tarifa completa;
				tarifaCompleta = tpValidacao.getTarifaFk().getValorTarifa();
			}
		}
		// Inseri no primeiro registro o valor da tarifa completa;
		tiposSelecionados.get(0).getTarifaFk().setValorTarifa(tarifaCompleta);
		return tiposSelecionados;
	}
