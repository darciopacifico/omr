...
public class FreemarkerTemplateRules implements Serializable, ITemplateRules {
	...
	/**
	 * Recupera regex do mapa de propriedades para determinar nome da entidade de domínio a partir do nome da classe java que servirá de
	 * insumo para geração de código. Exemplos: classe=PessoaVO.java => domínio=Pessoa; classe=ProdutoBean.java => domínio=Produto. <br>
	 * <br>
	 * O nome da entidade de domínio será igual ao grupo $1 da regex informada: Exemplo se classe=PessoaVO e domínio=Pessoa então
	 * Regex="(.*)(VO)", onde o primeiro par de parênteses corresponde ao grupo $1, de acordo com a implementação de regex de Java.
	 */
