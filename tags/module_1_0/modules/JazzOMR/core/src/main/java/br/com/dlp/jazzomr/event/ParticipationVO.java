...
public class ParticipationVO extends AbstractLogEntityVO<Long> {
	...
	public Map<AbstractCriterionVO, CriterionResultVO> getResultMap() {
		...
		if(resultMap==null){
			...
			for (CriterionResultVO criterionResultVO : criterionResults) {
				if(criterionResultVO!=null && criterionResultVO.getCriterionCoordinateVO()!=null){
					CriterionCoordinateVO criterionCoordinateVO = criterionResultVO.getCriterionCoordinateVO();
					AbstractCriterionVO abstractCriterionVO = criterionCoordinateVO.getAbstractCriterionVO();
					resultMap.put(abstractCriterionVO, criterionResultVO);
				}
			}
		}
	...
	protected boolean isAllPagesUploaded() {
		...
		Integer totalPages = getExamVariantVO().getExamVO().getRelatorioVO().getTotalPages();
	...
	public Map<Long, PayloadVO> getUniqueByPage() {
		...
		for (PayloadVO payloadVO : payloadVOs) {
			mapPayload.put((Long) payloadVO.getPK(), payloadVO);
		}
		return mapPayload;
	}
	...
	public Float getFulfilled() {
		...
		float totalPages = getExamVariantVO().getExamVO().getRelatorioVO().getTotalPages();
