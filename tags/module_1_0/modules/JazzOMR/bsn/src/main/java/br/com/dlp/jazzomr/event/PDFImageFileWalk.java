...
public class PDFImageFileWalk extends AbstractFileWalk {
	...
	public void processFile(byte[] bytes, IImageParserVisitor visitor) {
		...
		try {
			...
			throw new JazzRuntimeException("Erro ao tentar ler pdf contido no array de bytes!",e);
		}
		...
		for (Object object : pages) {
			...
			try {
				...
				throw new JazzRuntimeException("Erro ao tentar ler imagem contida na pagina de um pdf!",e);
			}
			if (images != null) {
				...
				while (imageIter.hasNext()) {
					...
					try {
						...
						throw new JazzRuntimeException("Erro ao tentar carregar imagem para bufferedImage!",e);
					}
				}
			}
		}
