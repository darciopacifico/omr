...
public class IJJazzOMR {
	...
	static Object runPlugIn(String commandName, String className, String arg) {
		if (IJJazzOMR.debugMode)
			IJJazzOMR.log("runPlugin: "+className+" "+arg);
		if (arg==null) arg = "";
	...
	static Object runUserPlugIn(String commandName, String className, String arg, boolean createNewLoader) {
		...
		if (checkForDuplicatePlugins) {
			...
			IJJazzOMR.runPlugIn("ij.plugin.ClassChecker", "");
	...
	public static void run(String command, String options) {
		System.out.println("testadn o hot deploy #################################################################");
	...
	static void init() {
		MenusJazzOMR m = new MenusJazzOMR(null, null);
		Prefs.load(m, null);
		m.addMenuBar();
	}
...
/** Clears the "Results" window and sets the column headings to
	...
	public static void renameResults(String title) {
		...
		if (frame!=null && (frame instanceof TextWindow)) {
			...
			if (tw.getTextPanel().getResultsTable()==null) {
				IJJazzOMR.error("Rename", "\""+tw.getTitle()+"\" is not a results table");
				return;
			}
			tw.rename(title);
	...
	public static void outOfMemory(String name) {
		...
		if (!memMessageDisplayed) {
			...
			log("<"+IJJazzOMR.URL+"/docs/install/>");
			log(">>>>>>>>>>>>>>>>>>>>>>>>>>>");
	...
	*/
	...
	public static void setAutoThreshold(ImagePlus img, String method) {
		...
		if (method!=null) {
			...
			catch (Exception e) {IJJazzOMR.log(e.getMessage());}
		} else
	...
	*/
	...
	public static ClassLoader getClassLoader() {
		if (classLoader==null) {
			...
			if (pluginsDir==null)
				return IJJazzOMR.class.getClassLoader();
			else {
				if (Menus.jnlp)
