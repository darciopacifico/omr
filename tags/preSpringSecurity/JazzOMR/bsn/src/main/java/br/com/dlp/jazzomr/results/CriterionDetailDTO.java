...
public class CriterionDetailDTO implements IBaseVO<String> {
	...
	public static final double COEF_AJUSTE_ALTURA = 1.21;
	...
	public CriterionDetailDTO(
			...
			CriterionResultVO criterionResultVO, PayloadVO payloadVO) {
		super();
		this.eventVO = eventVO;
		this.participationVO = participationVO;
		this.examVariantVO = examVariantVO;
	 ...
	 * @return
	 */
	public String drawMapCoords(CriterionCoordinateVO criterionCoordinateVO, double screenImgHeight){
			
		if(!(criterionCoordinateVO.getAbstractCriterionVO() instanceof DissertativeVO) || !isLoaded() ){
			return "";
		}
		
		
		int x1Link = 0;//pega desde a lateral da imagem
		int y1Link = getPosicaoY(criterionCoordinateVO, screenImgHeight);
		
		int widthLink = getWidthDissertative(criterionCoordinateVO, screenImgHeight);
		int heightLink = getHeightDissertative(criterionCoordinateVO, screenImgHeight);
		
		int y2Link = heightLink+y1Link;
		
		String coord =	x1Link+","+y1Link+","+widthLink+","+ y2Link;
	 ...
	 * @return
	 */
	protected int getWidthDissertative(CriterionCoordinateVO criterionCoordinateVO, double screenImgHeight) {
		
		double coefH = calcularCoefAjusteAltura(screenImgHeight);
		
		ImageVO imageVO = payloadVO.getImageVO();
		
		BufferedImage buffImage = imageVO.getImage();
		
		int imgWid = (int) (buffImage.getWidth()/coefH);
		return imgWid;
	}
	 ...
	 * @return
	 */
	protected int getHeightDissertative(CriterionCoordinateVO criterionCoordinateVO,double screenImgHeight) {
		double coefH = calcularCoefAjusteAltura(screenImgHeight);
		double h = (double)criterionCoordinateVO.getH();
		int hSize = (int)(h/coefH);
		return hSize;
	}
	...
	protected double getRadiusExamRef() {
		return getExamVO().determineTopLeftCorner().getX();
	}
	...
	protected double getDistRefY() {
		java.awt.geom.Point2D.Double tlPoint = getExamVO().determineTopLeftCorner();
		java.awt.geom.Point2D.Double brPoint = getExamVO().determineBottomRightCorner();
	...
	public StreamedContent streamExamImg() {
		...
		CriterionCoordinateVO criterionCoordinateVO = getCriterionCoordinateVO();

		if (payloadVO != null && payloadVO.getImageVO() != null) {

			if (criterionCoordinateVO.getAbstractCriterionVO() instanceof DissertativeVO) {
				streamedContent = streamDissertativeImg(criterionCoordinateVO, payloadVO);
			} else {
				streamedContent = streamAlternativeImg(criterionCoordinateVO, payloadVO);
			}

		} else {
			streamedContent = new DefaultStreamedContent(this.getClass().getResourceAsStream("/br/com/dlp/jazzomr/results/notSended.png"));
		}
	...
	public Boolean isLoaded(){
		return payloadVO!=null;
	}

	public Boolean isPayloadFetched(){
		return payloadVO!=null && payloadVO.getImageVO()!=null;
	}
	...
	protected BufferedImage getBufferedImage(CriterionCoordinateVO criterionCoordinateVO) {
		...
		ImageVO imageVO = payloadVO.getImageVOs().get(0);
