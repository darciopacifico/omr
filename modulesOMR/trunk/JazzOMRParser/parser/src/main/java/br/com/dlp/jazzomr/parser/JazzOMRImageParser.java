...
public class JazzOMRImageParser {
	...
	private static final double bulletMinCirc = 0.72;// ajuste de sensibilidade de reconhecimento de marcas
	...
	public List<QuestionResultVO> parseImages(EventVO eventVO, List<InputStream> images) throws JazzOMRException {
		...
		List<Future<List<QuestionResultVO>>> futures = new ArrayList<Future<List<QuestionResultVO>>>(images.size());
		
		for (InputStream inputStream : images) {
			
			
			JazzOMRParserCallable callable = new JazzOMRParserCallable(eventVO, inputStream, this);
			
			Future<List<QuestionResultVO>> future = executor.submit(callable);
		...
		try {
			executor.awaitTermination(images.size()*10, TimeUnit.SECONDS);
		} catch (InterruptedException e) {
			throw  new JazzOMRException("Erro ao tentar processar as imagens enviadas! "+e.getMessage(),e);
		}
		
		List<QuestionResultVO> resultsTotal = new ArrayList<QuestionResultVO>();
		
		for (Future<List<QuestionResultVO>> future : futures) {
			try {
				resultsTotal.addAll(future.get());
			} catch (Exception e) {
				throw  new JazzOMRException("Erro ao tentar processar as imagens enviadas! "+e.getMessage(),e);
			}
		}
	...
	public List<QuestionResultVO> parseImage(List<BufferedImage> images, EventVO eventVO) throws JazzOMRException {
		...
		for (BufferedImage bufferedImage : images) {
			...
			JazzOMRParserCallable callable = new JazzOMRParserCallable(eventVO, bufferedImage, this);
	 ...
	 * @throws JazzOMRException
	 */
	public List<QuestionResultVO> parseImage(BufferedImage image, EventVO eventVO) throws JazzOMRException {
		
		ImageDocVO imageDocVO = new ImageDocVO(image);
