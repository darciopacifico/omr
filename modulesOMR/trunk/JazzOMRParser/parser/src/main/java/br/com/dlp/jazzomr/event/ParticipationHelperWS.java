...
public class ParticipationHelperWS {
	 ...
	 * 
	 */
	public EventProcessDTO processParticipations(String eventName, ExamVO examVO, List<PessoaVO> pessoas, Integer numeroDeVariacoes, JasperReport templateProvaJR) {
		...
		JasperPrint eventoReturn = fillExamVariantsCoordinates(eventVO, numeroDeVariacoes, templateProvaJR);
				
		EventProcessDTO eventDTO = new EventProcessDTO();
		
		eventDTO.setJasperPrint(eventoReturn);
		eventDTO.setEventVO(eventVO);
	...
	protected String getCoordinateKey(Map<String, OMRCoordinatesVO> coordinateMap, QuestionnaireVO questionnaireVO, QuestionVO questionVO, CriterionVO criterionVO, Integer examVar) {
		...
		if(questionPK==null || pergPK==null || critPK==null){
			throw new JazzOMRRuntimeException("Os codigos de questionario, pergunta e criterio (alternativo/dissertativo)" +
		...
		if(coordinateMap.containsKey(coordinateKey)){
			throw new JazzOMRRuntimeException("O codigo de coordenada composto por questionario+pergunta+criterio deve ser unico."+
	 ...
	 * @return 
	 */
	private JasperPrint fillExamVariantsCoordinates(EventVO eventVO, Integer numeroDeVariacoes, JasperReport templateProvaJR) {
		...
		try {
			...
			throw new JazzOMRRuntimeException("erro ao tentar parsear xml resultado", e);
		} catch (IOException e) {
			throw new JazzOMRRuntimeException("erro ao tentar parsear xml resultado", e);
		}
	...
	private SAXParser createSAXParser() {
		...
		try {
			...
			throw new JazzOMRRuntimeException("erro ao tentar criar um sax parser", e);
		} catch (SAXException e) {
			throw new JazzOMRRuntimeException("erro ao tentar criar um sax parser", e);
		}
		return saxParser;
	...
	private JasperPrint createJasperPrint(EventVO eventVO, Integer numeroDeVariacoes, JasperReport templateProvaJR) {
		...
		try {
			...
			throw new JazzOMRRuntimeException("Erro ao preencher relatorio", e);
		}
