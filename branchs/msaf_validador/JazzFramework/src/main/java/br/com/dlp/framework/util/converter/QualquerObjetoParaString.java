	...
	public Object convert(Class arg0, Object valorOrigem) {
		...
		try {
			...
			if (valorOrigem instanceof String) {
				...
				returnValue = (String) (new DateParaStringConverter(
						getResourceBundle()).convert(arg0, valorOrigem));
				...
				DecimalFormat decimalFormat = new DecimalFormat(
						getFormatoPadrao(valorOrigem), decimalFormatSymbols);

				
				

				if (valorOrigem instanceof Short) {
					returnValue = decimalFormat.format(((Short) valorOrigem)
							.shortValue());

				} else if (valorOrigem instanceof Integer) {
					returnValue = decimalFormat.format(((Integer) valorOrigem)
							.intValue());

				} else if (valorOrigem instanceof Long) {
					returnValue = decimalFormat.format(((Long) valorOrigem)
							.longValue());

				} else if (valorOrigem instanceof Float) {
					returnValue = decimalFormat.format(((Float) valorOrigem)
							.floatValue());

				} else if (valorOrigem instanceof Double) {
					returnValue = decimalFormat.format(((Double) valorOrigem)
							.doubleValue());
			...
			logger.error("Erro ao tentar converter o objeto '" + valorOrigem
					+ "'!", e);
			e.printStackTrace();
			throw new RuntimeException("Erro ao tentar converter o objeto '"
					+ valorOrigem + "'!", e);
