...
public class SwingConsumerMonitor extends JFrame {
	...
	protected void adicionaEventosAosBotaoes() {
		btnIniciar.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(log.isDebugEnabled()) log.debug("Erro ao tentar consumir fila JMS");
		...
		comboBoxJMSProxy.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				try {
					proxySelecionado = comboBoxJMSProxy.getSelectedItem().toString();
					if(log.isDebugEnabled()) log.debug("proxy selecionado:"+ proxySelecionado);
					txtConsole.setText("proxy selecionado:"+ proxySelecionado);
				} catch (Exception e2) {
					if(log.isDebugEnabled()) log.debug("Erro ao tentar conectar ao proxy:"+proxySelecionado);
					JOptionPane msgErro = new JOptionPane("Teste!");
		...
		comboBoxDllConfProxy.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(log.isDebugEnabled()) log.debug("dll config selecionado:"+ dllConfSelecionado.getProxyHost());
					txtConsole.setText("config DLL selecionada:"+ dllConfSelecionado);
				} catch (Exception e2) {
					if(log.isDebugEnabled()) log.debug("Erro ao tentar conectar a dll:"+dllConfSelecionado);
					JOptionPane msgErro = new JOptionPane("Teste!");
					msgErro.showConfirmDialog(estaTela, "Erro: Confirma");
				}
			}
	...
	protected void recuperaValidadorConsumidorBO() {
		try {
			...
			if(log.isDebugEnabled()) log.debug("Erro ao tentar inicializar consumo de mensagens JMS");
			System.exit(0);
		}
	}
