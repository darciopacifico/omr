...
public class GeradorXmlCRM implements GeradorXmlPedidoValidacao {
	...
	public String gerarXml(Object... argumentos) {
		...
		return MessageFormat.format(
				"<?xml version=\"1.0\" encoding=\"iso-8859-1\"?> "
						...
						+ "<CRM>{0}</CRM>" + "<Estado>{1}</Estado>" + "<Proxy>"
						+ "<Ativo>{2}</Ativo>" + "<Usuario>{3}</Usuario>"
						+ "<Senha>{4}</Senha>" + "<Host>{5}</Host>"
						+ "<Porta>{6}</Porta>" + "</Proxy>"
						+ "</ParametrosConsulta>", 
				documento, 
				estado,
