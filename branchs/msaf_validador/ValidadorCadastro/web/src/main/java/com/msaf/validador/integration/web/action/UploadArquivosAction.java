...
public class UploadArquivosAction extends BaseAction {
	 ...
	 * @return
	 */
	public ActionForward doCancelCotacao(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)  {
		
		request.getSession().removeAttribute(UploadArquivosAction.BYTES_ARQUIVO_VALIDACAO_UPLOAD);
	...
	public ActionForward doPopUp(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		PedValidacaoVO pedidoValidacao = pedidoValidacaoServer.getPedidoValidacao(validaIDPedValidacao(request));
		
		uploadArquivosForm.setListPessoas(pedidoValidacaoServer.getListaPessoaPorPedidoValidacao(pedidoValidacao));
	...
	private void getListaPedidosValidacaoCliente(ActionForm form, HttpServletRequest request) throws ValidadorException {
		...
		pageString = Util.isEmpty(pageString)? "0": pageString;
		
		page = Integer.parseInt(pageString);

		uploadArquivosForm.setPage(page);
		
		PesquisaPedidoValidacao pesquisaPedidoValidacao = new PesquisaPedidoValidacao();
		
		pesquisaPedidoValidacao.setPedidoValidacao(new PedValidacaoVO());
		pesquisaPedidoValidacao.getPedidoValidacao().setClienteFk(cliente);
		
		if(!Util.isEmpty(uploadArquivosForm.getDataFimSolicitacao())) {
			pesquisaPedidoValidacao.setDataFimSolicitacao(DateUtil.getDate(uploadArquivosForm.getDataFimSolicitacao() + " 23:59:59 ",DateUtil.FORMATACAO_DATA_HORA_MINUTO_SEGUNDO));
		}
		
		if(!Util.isEmpty(uploadArquivosForm.getDataInicioSolicitacao())) {
			pesquisaPedidoValidacao.setDataInicioSolicitacao(DateUtil.getDate(uploadArquivosForm.getDataInicioSolicitacao() + " 00:00:00",DateUtil.FORMATACAO_DATA_HORA_MINUTO_SEGUNDO));
		}
		
		uploadArquivosForm.setQuantidadeRegistros(getPedidoValidacaoServer().countListaPedidosValidacaoCliente(pesquisaPedidoValidacao));
		
		uploadArquivosForm.setListPedidosValidacao(getPedidoValidacaoServer().listaPedidoValidacaoCliente(pesquisaPedidoValidacao, page -1));
	}
