...
public class GeradorXLSRetorno {
	...
	private HSSFRow criarCorpoArquivo(HSSFWorkbook wb, HSSFSheet sheet, int numLinha, DadosRetInstVO dadosRetornoInstituicaoVO) {
		...
		// recupera o tipo de documento em que foi realizado a consulta {0 - cpf, 1 - cnpj}
		row.createCell(i++, HSSFCell.CELL_TYPE_STRING).setCellValue(""+this.getTipoDocumento(dadosRetornoInstituicaoVO));	
	...
	/**
	 * Recupera o tipo de documento em que foi realizado a consulta.
	 * @param dadosRetornoInstituicaoVO
	 * @return
	 * @author Ekler Paulno de Mattos
	 */
	private String getTipoDocumento( DadosRetInstVO dadosRetornoInstituicaoVO){
		final String IE = "IE";
		final String CNPJ = "CNPJ";

		final String NUM_CNPJ = "0";
		final String NUM_IE = "1";

		
		String tipoDocumento = new String();
		tipoDocumento = dadosRetornoInstituicaoVO.getTipoDocumento();
		if(tipoDocumento== null)
			return "";
		else if(tipoDocumento.equals(NUM_CNPJ))
			return CNPJ;
		
		else if (tipoDocumento.equals(NUM_IE))
			return IE;
		
		else return tipoDocumento;
	}
